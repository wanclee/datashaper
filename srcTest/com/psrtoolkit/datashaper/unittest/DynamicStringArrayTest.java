...
public class DynamicStringArrayTest {
    ...
    public void testIntialization() {
        ...
        DynamicStringArray arr2 = new DynamicStringArray(200);
        assertTrue(arr2.length() == 200);
        arr2.add("999L");
        assertTrue(arr2.lastpos() == 1);
        assertTrue(arr2.get(0).equals("999L"));
    }

    @Test
    public void testIntializationNegative() {
        try {
            DynamicStringArray arr = new DynamicStringArray(-300);
    ...
    @Test
    public void testGet() {
        DynamicStringArray arr = new DynamicStringArray(200);
        for (int i = 0; i < arr.length(); i++) {
            arr.add(Integer.toString(i + 399));
        }
        assertTrue(arr.length() == 200);
        assertTrue(arr.lastpos() == 200);

        for (int k = 0; k < arr.length(); k++) {
            assertTrue(arr.get(k).equals(Integer.toString(k + 399)));
        }
        assertTrue(arr.length() == 200);
        assertTrue(arr.lastpos() == 200);
    }
    ...
    public void testSet() {
        DynamicStringArray arr = new DynamicStringArray(200);
        for (int i = 0; i < arr.length(); i++) {
            arr.set(i, Integer.toString(i + 399));
        }
        assertTrue(arr.length() == 200);
        assertTrue(arr.lastpos() == 200);
        int len = arr.length();
        for (int k = 0; k < 100; k++) {
            arr.set(len + k, Integer.toString(k + 399));
        }
    ...
    public void testSetPositive2() {
        ...
        int len = arr.length();
        arr.set(len + 400, Integer.toString(99999));
    ...
    public void testSetNegative() {
        ...
        try {
            arr.set(-777, Integer.toString(9999));
    ...
    public void testAdd() {
        ...
        for (int k = 0; k < 100; k++) {
            arr.add(Integer.toString(k + 399));
        }
        assertTrue(arr.length() == 300); //proved that the array grows
        assertTrue(arr.lastpos() == 300);
    }
